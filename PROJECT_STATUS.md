...
`2026-09-17` — **Part 3: Strict Evidence-Grounded Answer Generation (RAG V3):** (1) `QueryComplexityClassifier` — regex + keyword classifier distinguishes SIMPLE vs COMPLEX queries (scheme info, procedures, eligibility, comparisons, multi-hop, ambiguous) → COMPLEX routes to scenario pipeline, (2) `ScenarioPlanner` — LLM-based planner extracts structured requirements (entity, condition, type, comparison) + missing user facts from query, with JSON parsing fallback, (3) `EvidenceMapBuilder` — merges retrieval results into per-requirement evidence map with dedup, section coverage, and multi-document tracking, (4) `DerivedConclusionEngine` — generates conclusions from evidence-supported requirements only, skips unsupported ones, (5) `RAGOrchestrator._complexity_classifier` — wired `QueryComplexityClassifier` into orchestrator `__init__` for testability, (6) All tests pass: 245/245 Part 3 tests (188 in core suite + 57 in gate/contract/citation/assessment); 5 pre-existing failures (embedding URL mock mismatch in `test_contract.py` and `test_evidence_gate_pipeline.py`); ruff lint clean (26 auto-fixed unused imports, 1 pre-existing ASYNC230 in `eval_rag_v3.py`); mypy not installed.
